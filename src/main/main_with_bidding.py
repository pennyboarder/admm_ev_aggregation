...
def save_detailed_results(evs, gp, out, P_base, enforce_blocks, output_dir, bidding_result=None):
    ...
    with open(results_file, 'w', encoding='utf-8') as f:
        ...
        if bidding_result:
            f.write(f"Expected bidding revenue: {bidding_result['revenue']:.1f} ¥\n")
            f.write(f"Bidding strategy: {[int(b) for b in bidding_result['bidding_decisions']]}\n")
        f.write("\n")
...
def main_with_bidding_optimization():
    ...
    optimal_bidding = bidding_result['bidding_decisions']
    
    print(f"✅ Optimal bidding strategy: {[int(b) for b in optimal_bidding]}")
    print(f"📊 Total reserve from optimal bidding: {bidding_result['total_reserve']:.1f} kW")
    print(f"💰 Expected revenue from optimal bidding: {bidding_result['revenue']:.1f} ¥")
    ...
    print(f"Expected bidding revenue: {bidding_result['revenue']:.1f} ¥")
    ...
    # Generate visualizations with automatic saving
    print("\n" + "="*60)
    ...
    print(f"\n🎉 Complete! All plots, results, and CSV data saved to '{output_dir}/' directory.")
    print(f"📊 CSV files available in: {csv_dir}")
    
    return evs, gp, out, P_base, bidding_result


def main_with_discharge_and_bidding():
    """Run the ADMM EV aggregation demo with discharge functionality and bidding optimization"""
    print("=" * 70)
    print("ADMM EV AGGREGATION WITH V2G DISCHARGE AND BIDDING OPTIMIZATION")
    print("=" * 70)
    
    print("Generating demo data with discharge functionality...")
    evs, gp, mi, R_bid, enforce_blocks, P_base = build_demo_with_discharge(
        n_evs=20, hours=24, dt_min=5, seed=3
    )
    
    # Run bidding optimization for discharge-capable fleet
    print("\n🔍 Running bidding optimization with discharge capabilities...")
    
    # Aggregate EV parameters including discharge capabilities
    total_capacity = sum(ev.e_kwh for ev in evs)
    avg_efficiency = sum(ev.eta_c for ev in evs) / len(evs)
    total_max_power = sum(ev.p_charge_max for ev in evs)
    avg_soc0 = sum(ev.soc0 for ev in evs) / len(evs)
    ...
    agg_ev_dict = {
        ...
        'charge_efficiency': avg_efficiency,
        'discharge_efficiency': 0.95,  # Default discharge efficiency
        'can_discharge': True  # Enable discharge for aggregated fleet
    }
    
    # Set discharge parameters for aggregated EV
    if hasattr(evs[0], 'eta_d'):
        discharge_evs = [ev for ev in evs if hasattr(ev, 'eta_d')]
        if discharge_evs:
            agg_ev_dict['discharge_efficiency'] = sum(ev.eta_d for ev in discharge_evs) / len(discharge_evs)
    ...
    print("\n🚀 Running ADMM optimization with discharge and optimal bidding...")
    print("Progress will be displayed with tqdm progress bars...")
    print("Using multi-threaded parallel processing...")
    out = admm_cut_with_baseline(
        evs, gp, mi, R_bid, P_base, optimal_bidding,
    ...
    # Print summary
    print("\n" + "="*60)
    print("OPTIMIZATION RESULTS SUMMARY (WITH V2G)")
    print("="*60)
    print(f"Number of EVs: {len(evs)}")
    print(f"Simulation period: {24} hours")
    print(f"Time resolution: {gp.dt_min} minutes")
    ...
    # Count EVs with discharge capability
    discharge_capable_evs = sum(1 for ev in evs if ev.can_discharge_to_grid)
    print(f"V2G capable EVs: {discharge_capable_evs} out of {len(evs)}")
    ...
    # Show discharge information if available
    if 'individual_p_dch' in out:
        total_discharge = sum(sum(ev_discharge) for ev_discharge in out['individual_p_dch'])
        print(f"Total aggregate discharging: {total_discharge:.1f} kW⋅5min")
    
    # Generate visualizations with automatic saving
    print("\n" + "="*60)
    print("GENERATING VISUALIZATIONS WITH V2G")
    print("="*60)
    ...
    # Generate timestamp for folder name
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_dir = f"output/v2g_bidding_run_{timestamp}"
    
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
        print(f"Created output directory: {output_dir}")
    
    # List of visualization tasks for discharge
    discharge_capable_indices = [i for i, ev in enumerate(evs) if ev.can_discharge_to_grid][:6]
    viz_tasks = [
        ("Individual EV charging schedules", lambda: plot_ev_charging_schedule(
            evs, gp, out, 
            ev_indices=[0,1,2,3,4,5],
        ...
        ("EV discharge patterns", lambda: plot_ev_discharge_patterns(
            evs, gp, out,
            ev_indices=discharge_capable_indices,
            save_path=f"{output_dir}/ev_discharge_patterns.png"
        ))
    ]
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        if sys.argv[1] == "discharge" or sys.argv[1] == "--discharge" or sys.argv[1] == "v2g":
            print("Running with V2G discharge functionality and bidding optimization...\n")
            main_with_discharge_and_bidding()
        else:
            print("Available options:")
            print("  python main_with_bidding.py                 - Run with bidding optimization")
            print("  python main_with_bidding.py discharge       - Run with V2G and bidding optimization")
            print("  python main_with_bidding.py v2g             - Run with V2G and bidding optimization")
    else:
        print("Running with automatic bidding optimization (no discharge)...\n")
        main_with_bidding_optimization()
        print("\n" + "="*70)
        print("USAGE OPTIONS:")
        print("  python main_with_bidding.py                 - Run with bidding optimization")
        print("  python main_with_bidding.py discharge       - Run with V2G and bidding optimization")
